"""
🤖 DENIA AI BOT - FINAL PRO VERSION
Admin quan ly hoan toan qua Telegram | Deep Link dich | AI Memory + Personality
Tinh nang: Daily Checkin, Referral, Leaderboard, Maintenance, Auto Cleanup, Export
Chi can: pip install -r requirements.txt && python run.py
"""
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Default prompts
        await db.execute("INSERT OR IGNORE INTO prompts (name, content) VALUES (?, ?)",
            ("default", SYSTEM_PROMPT_DEFAULT))
        await db.execute("INSERT OR IGNORE INTO prompts (name, content) VALUES (?, ?)",
            ("creative", "Ban la Denia AI Creative - Nha van, nghe si sang tao. Tra loi bay bong, giau cam xuc, dung an du, tu ngu my le."))
        await db.execute("INSERT OR IGNORE INTO prompts (name, content) VALUES (?, ?)",
            ("coder", "Ban la Denia AI Coder - Lap trinh vien cao cap. Tra loi bang code, giai thich ky thuat chinh xac, dung markdown, neu vi du ro rang."))
        await db.execute("INSERT OR IGNORE INTO prompts (name, content) VALUES (?, ?)",
            ("teacher", "Ban la Denia AI Teacher - Giao vien kien nhan. Giai thich tu co ban den nang cao, dung vi du thuc te, khuyen khich nguoi hoc."))
        await db.commit()
...
async def can_create_key(user_id: int) -> tuple:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not row:
            return (True, "")
        if row[3]:
            return (False, "🚫 Tai khoan cua ban da bi khoa. Lien he admin.")
...
async def create_key(user_id: int, key_code: str) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        try:
            await db.execute("INSERT INTO keys (key_code, user_id, expires_at) VALUES (?, ?, ?)",
                (key_code, user_id, expires.isoformat()))
            today_str = datetime.now().strftime("%Y-%m-%d")
            await db.execute("""
            ...
            logger.error("create_key error: " + str(e))
            return False
...
async def use_key(key_code: str, user_id: int) -> dict:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if used:
            return {"ok": False, "msg": "❌ Key da duoc su dung."}
        if key_owner != user_id:
            return {"ok": False, "msg": "❌ Key nay khong thuoc ve ban."}
        ...
        if bal + req_per > max_bal:
            return {"ok": False, "msg": "💎 Ban da dat gioi han " + str(max_bal) + " req."}
        await db.execute("UPDATE keys SET used = 1 WHERE key_code = ?", (key_code,))
        await db.execute("UPDATE users SET req_balance = req_balance + ?, total_keys_used = total_keys_used + 1 WHERE user_id = ?",
            (req_per, user_id))
        await db.execute("INSERT INTO logs (user_id, action, detail) VALUES (?, ?, ?)",
            (user_id, "USE_KEY", key_code + " +" + str(req_per) + "req"))
        await db.commit()
...
async def get_prompt_content(name: str) -> str:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        row = await cur.fetchone()
        return row[0] if row else SYSTEM_PROMPT_DEFAULT
...
async def add_conversation(user_id: int, role: str, content: str):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cfg = await get_all_runtime_config()
        max_mem = int(cfg.get("MAX_MEMORY_MESSAGES", MAX_MEMORY_MESSAGES))
        cur = await db.execute("SELECT id FROM conversations WHERE user_id = ? ORDER BY id DESC LIMIT ? OFFSET ?",
            (user_id, 1000, max_mem))
...
async def set_model_price(model_name: str, price: int, enabled: int = 1):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR REPLACE INTO model_prices (model_name, req_price, enabled) VALUES (?, ?, ?)",
            (model_name, price, enabled))
        await db.commit()
        ...
        if count == 0:
            for k, v in os.environ.items():
                if k.startswith("PRICE_"):
                    ...
                    await db.execute("INSERT INTO model_prices (model_name, req_price, enabled) VALUES (?, ?, ?)",
                        (model_name, int(v), 1))
            await db.commit()
...
async def get_stats() -> dict:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        total_req = (await cur.fetchone())[0] or 0
        ...
        pending_keys = (await cur.fetchone())[0]
...
async def admin_add_req(user_id: int, amount: int):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("INSERT INTO logs (user_id, action, detail) VALUES (?, ?, ?)",
            (user_id, "ADMIN_ADD", "+" + str(amount) + "req"))
        await db.commit()
...
async def get_user_logs(user_id: int, limit: int = 20) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT action, detail, created_at FROM logs WHERE user_id = ? ORDER BY id DESC LIMIT ?",
            (user_id, limit))
        return await cur.fetchall()
...
async def get_pending_feedback(limit: int = 20) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("""
            ...
            WHERE f.status = 'pending' ORDER BY f.id DESC LIMIT ?
        """, (limit,))
        return await cur.fetchall()
...
async def check_referral(referrer_id: int, referred_id: int) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if await cur.fetchone():
            return False
        await db.execute("INSERT INTO referrals (referrer_id, referred_id, bonus_given) VALUES (?, ?, 1)",
        ...
        await db.execute("INSERT INTO logs (user_id, action, detail) VALUES (?, ?, ?)",
            (referred_id, "REFERRAL", "Invited by " + str(referrer_id) + ", +" + str(bonus) + "req"))
        await db.commit()
        return True
...
async def daily_checkin(user_id: int) -> tuple:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("UPDATE users SET daily_checkin_date = ?, req_balance = req_balance + ? WHERE user_id = ?",
            (today, bonus, user_id))
        await db.execute("INSERT INTO logs (user_id, action, detail) VALUES (?, ?, ?)",
            (user_id, "CHECKIN", "+" + str(bonus) + "req"))
        await db.commit()
        return (True, bonus)
...
# ========================
# Link Dich Generator (Cach 1: Telegram Deep Link)
# ========================
async def create_link_dich(bot_username: str, key: str) -> tuple[str, str]:
    if bot_username:
        tg_link = "https://t.me/" + bot_username + "?start=" + key
        return (tg_link, "telegram_deep_link")
    return ("", "none")
...
async def chat_ai(user_id: int, user_message: str) -> tuple[str, int]:
    ...
    try:
        ...
        return ("❌ AI dang ban. Vui long thu lai sau.", 0)
...
async def auto_set_admin(user_id: int):
    ...
    if ADMIN_ID == 0:
        ADMIN_ID = user_id
        logger.info("Auto-set admin: " + str(user_id))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Giai phap 1: Telegram Deep Link - auto verify key tu start parameter
        ...
        if key_input.startswith("DENIA-"):
            ...
            if result["ok"]:
                await update.message.reply_text(
                    "✅ <b>Chao mung tro lai!</b>\n" + result["msg"] + "\nHay bat dau chat AI ngay bay gio.",
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📅 Diem danh hom nay", callback_data="checkin")],
        [InlineKeyboardButton("🤖 Chon model AI", callback_data="models")],
        [InlineKeyboardButton("🎭 Chon tinh cach", callback_data="prompts")],
        [InlineKeyboardButton("💎 Xem so du & Profile", callback_data="balance")],
        [InlineKeyboardButton("🏆 Bang xep hang", callback_data="top")],
        [InlineKeyboardButton("📞 Ho tro Admin", callback_data="support")]
    ]

    await update.message.reply_text(
        "👋 Chao <b>" + html.escape(user.first_name or "ban") + "</b>!\n\n"
        "🤖 <b>Denia AI Bot</b> - Chat AI thong minh, co nho!\n"
        "💎 So du: <b>" + str(u['req_balance']) + " req</b>\n"
        "⚙️ Model: <code>" + html.escape(u['selected_model'] or DEFAULT_MODEL) + "</code>\n"
        "🎭 Tinh cach: <code>" + str(u['selected_prompt']) + "</code>\n"
        "💰 Gia chat: <b>" + str(get_model_price(u['selected_model'] or DEFAULT_MODEL)) + " req</b>/tin nhan\n\n"
        "<b>📋 Bang gia model:</b>\n" + price_list + "\n\n"
        "💡 Go <code>/new</code> de xoa lich su chat.\n"
        "🎁 Ma gioi thieu: <code>" + str(u['referral_code']) + "</code>\n"
        "Nhan <b>🔑 Nhan key</b> de vuot link va nhan req.",
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 <b>Huong dan Denia AI</b>\n\n"
        "1️⃣ Nhan 🔑 <b>Nhan key moi</b>\n"
        "2️⃣ Bot tao key + link Telegram Deep Link\n"
        "3️⃣ Bot tu rut gon link qua Yeumoney\n"
        "4️⃣ Vuot link → Click link Telegram → Nhan Start → Bot tu dong nhan key!\n"
        "5️⃣ Khong can go /key thu cong!\n\n"
        "🎭 <b>Lenh nang cao:</b>\n"
        "• <code>/model</code> - Chon model AI\n"
        "• <code>/prompt</code> - Chon tinh cach\n"
        ...
        "⚠️ Moi tin nhan AI ton req tuy model (1-5 req)\n"
        "⏳ Nhan key cach 15 phut, toi da 10 key/ngay\n"
...
async def key_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not key_input.startswith("DENIA-"):
        await update.message.reply_text("❌ Key phai bat dau bang <code>denia-</code>", parse_mode=ParseMode.HTML)
        return
    ...
    for k in prices.keys():
        if model_name.lower() in k.lower() or k.lower() in model_name.lower():
            if prices[k]["enabled"]:
                ...
                break
    if not found:
        await update.message.reply_text("❌ Model khong hop le hoac da bi tat.")
        return
    await set_user_model(user_id, model_name)
    await update.message.reply_text(
        "✅ Da chuyen sang model: <code>" + html.escape(model_name) + "</code>\n"
        "💰 Gia: " + str(prices[model_name]['price']) + " req/tin nhan",
        parse_mode=ParseMode.HTML
    )
...
async def prompt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        for name, content, enabled in prompts:
            if enabled:
                buttons.append([InlineKeyboardButton(name.upper(), callback_data="setprompt|" + name)])
        await update.message.reply_text(
    ...
    if name not in valid:
        await update.message.reply_text("❌ Tinh cach khong hop le. Cac lua chon: " + ", ".join(valid))
        return
    await set_user_prompt(user_id, name)
    await update.message.reply_text(
        "🎭 Da chuyen sang tinh cach: <b>" + name.upper() + "</b>\n"
        "AI se tra loi theo phong cach moi tu tin nhan tiep theo.",
        parse_mode=ParseMode.HTML
    )
...
async def history_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not hist:
        await update.message.reply_text("📝 Chua co lich su chat nao.")
        return
...
async def top_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not top:
        await update.message.reply_text("🏆 Chua co du lieu xep hang.")
        return
...
async def checkin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        await update.message.reply_text(
            ...
            "💎 Ban nhan +" + str(bonus) + " req. Hen gap lai ngay mai!",
            parse_mode=ParseMode.HTML
        )
    else:
        await update.message.reply_text(
            "⚠️ Ban da diem danh hom nay roi.\n"
            "Hay quay lai vao ngay mai nhe!",
            parse_mode=ParseMode.HTML
        )
...
async def ref_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Ma: <code>" + str(u['referral_code']) + "</code>\n"
        "Link: <code>" + link + "</code>\n\n"
        "💎 Moi luot moi thanh cong, ca 2 deu nhan +" + str(bonus) + " req!\n"
        "Chia se link ben tren cho ban be nhe.",
        parse_mode=ParseMode.HTML
    )
...
async def feedback_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "Vi du: <code>/feedback Bot rat hay, cam on admin!</code>",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    await update.message.reply_text(
        "✅ Cam on ban da gop y!\n"
        "Admin se xem xet va phan hoi som nhat.",
        parse_mode=ParseMode.HTML
    )
    try:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "getkey":
        ...
        if not ok:
            await query.edit_message_text("⚠️ " + msg, parse_mode=ParseMode.HTML)
            return
        key = generate_key()
        await create_key(user_id, key)

        bot_username = context.bot.username
        link_dich, method = await create_link_dich(bot_username, key)
        short_link = None
        if link_dich:
            short_link = await shorten_yeumoney(link_dich)

        cfg = await get_all_runtime_config()
        expire = int(cfg.get("KEY_EXPIRE_MINUTES", KEY_EXPIRE_MINUTES))

        if short_link:
            link_display = short_link
            note = "✅ Bot da tu dong rut gon link."
        else:
            link_display = link_dich
            note = "⚠️ Bot chua rut gon duoc. Ban can tu rut gon qua Yeumoney."

        await query.edit_message_text(
            "🔑 <b>Key cua ban:</b> <code>" + key + "</code>\n\n"
            "🔗 <b>Link:</b> " + html.escape(link_display or "(Loi)") + "\n\n"
            + note + "\n\n"
            "📋 <b>Cach vuot:</b>\n"
            "1. Click link rut gon ben tren\n"
            "2. Vuot Yeumoney (cho 15-30s)\n"
            "3. Sau khi vuot xong, click link Telegram\n"
            "4. Nhan <b>Start</b> → Bot tu dong nhan key!\n\n"
            "⏳ Key het han sau " + str(expire) + " phut.\n"
            "💡 Neu ban dang o Telegram, hay copy link rut gon sang trinh duyet de vuot.",
        ...
        for k,v in prices.items():
            if k.lower() in model.lower() or model.lower() in k.lower():
                ...
                break
        await query.edit_message_text(
            "💎 <b>So du:</b> " + str(u['req_balance']) + " req\n"
            "🔑 Da dung: " + str(u['total_keys_used']) + " key\n"
        ...
        await query.edit_message_text(
            "🤖 <b>Chon model AI:</b>\n\n"
            "Model re = nhanh, don gian.\n"
            "Model dat = thong minh, chuyen sau.",
            reply_markup=InlineKeyboardMarkup(buttons),
            parse_mode=ParseMode.HTML
        )
        ...
        await query.edit_message_text(
            ...
            "• DEFAULT - Tro ly can bang\n"
            "• CREATIVE - Bay bong, sang tao\n"
        ...
        top = await get_top_users()
        if not top:
            await query.edit_message_text("🏆 Chua co du lieu xep hang.")
            return
        text = "🏆 <b>Bang xep hang - Top 10</b>\n\n"
        medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]
        for i, u in enumerate(top):
        ...
        await query.edit_message_text(
            "✅ Da chuyen sang model: <code>" + html.escape(model) + "</code>\n"
            "💰 Gia: " + str(price) + " req/tin nhan\n\n"
            "Bat dau chat ngay bay gio!",
            parse_mode=ParseMode.HTML
        )
...
async def chat_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_maintenance() and not is_admin(user_id):
        await update.message.reply_text(
            ...
            "Vui long quay lai sau. Ban van co the nhan key va diem danh.",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    if not success:
        ...
        await update.message.reply_text(
            "⚠️ Ban da het req (can " + str(price) + " req de chat model nay).\n"
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "💎 Tong req dang luu hanh: " + str(stats['total_req']) + "\n"
        "🔑 Tong key da dung: " + str(stats['total_keys']) + "\n"
        "⏳ Key cho xu ly: " + str(stats['pending_keys']) + "\n"
        "💬 Tin nhan AI: " + str(stats['total_messages']) + "\n"
        "📅 Hoat dong hom nay: " + str(stats['today_logs']) + "\n"
        "📣 Feedback cho xu ly: " + str(stats['pending_feedback']) + "\n\n"
        "📞 Admin: " + ADMIN_PHONE,
        parse_mode=ParseMode.HTML
    )
...
async def user_detail_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    if not context.args:
        await update.message.reply_text("⚠️ Vui long nhap user ID. Vi du: <code>/user 123456789</code>", parse_mode=ParseMode.HTML)
        return
    try:
    ...
    logs = await get_user_logs(target_id, 10)
    text = (
...
async def addreq_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("⚠️ Vi du: <code>/addreq 123456789 100</code>", parse_mode=ParseMode.HTML)
        return
    try:
        target_id = int(context.args[0])
    ...
    try:
        await context.bot.send_message(
            ...
            "So du hien tai da duoc cap nhat.",
            parse_mode=ParseMode.HTML
        )
    except Exception:
        pass
...
async def ban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    if not context.args:
        await update.message.reply_text("⚠️ Vi du: <code>/ban 123456789</code>", parse_mode=ParseMode.HTML)
        return
    try:
        target_id = int(context.args[0])
    except ValueError:
...
async def unban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    if not context.args:
        await update.message.reply_text("⚠️ Vi du: <code>/unban 123456789</code>", parse_mode=ParseMode.HTML)
        return
    try:
        target_id = int(context.args[0])
    except ValueError:
...
async def config_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    cfg = await get_all_runtime_config()
    text = "⚙️ <b>Cau hinh hien tai:</b>\n\n"
    for k, v in cfg.items():
        text += "• <code>" + html.escape(k) + "</code> = <code>" + html.escape(v) + "</code>\n"
    await update.message.reply_text(text, parse_mode=ParseMode.HTML)
...
async def setprice_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    if len(context.args) < 2:
        await update.message.reply_text("⚠️ Vi du: <code>/setprice deepseek-v4-flash 2</code>", parse_mode=ParseMode.HTML)
        return
    model = context.args[0].strip()
    try:
        price = int(context.args[1])
    except ValueError:
        await update.message.reply_text("❌ Gia phai la so.")
        return
...
async def togglemodel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    if not context.args:
        await update.message.reply_text("⚠️ Vi du: <code>/togglemodel deepseek-v4-flash</code>", parse_mode=ParseMode.HTML)
        return
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        row = await cur.fetchone()
        if not row:
            await update.message.reply_text("❌ Model khong ton tai.")
            return
...
async def addprompt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    if len(context.args) < 2:
        await update.message.reply_text("⚠️ Vi du: <code>/addprompt funny Ban la hai huoc...</code>", parse_mode=ParseMode.HTML)
        return
    name = context.args[0].strip().lower()
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    if not context.args:
        await update.message.reply_text("⚠️ Vi du: <code>/broadcast Chao tat ca! Co tin moi...</code>", parse_mode=ParseMode.HTML)
        return
    ...
    for uid in users:
        try:
            await context.bot.send_message(uid, "📢 <b>Thong bao tu Admin:</b>\n\n" + message, parse_mode=ParseMode.HTML)
            sent += 1
...
async def health_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        status.append("🔴 Database: " + str(e))
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(AI_BASE_URL.replace("/v1/chat/completions", "/v1/models"),
                headers={"Authorization": "Bearer " + AI_API_KEY})
            if r.status_code in [200, 401, 403]:
                status.append("🟢 AI API: OK")
            else:
                status.append("🟡 AI API: HTTP " + str(r.status_code))
    except Exception as e:
        status.append("🔴 AI API: " + str(e))
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            import urllib.parse
            ...
            if r.status_code == 200:
                status.append("🟢 Yeumoney API: OK")
            else:
                status.append("🟡 Yeumoney API: HTTP " + str(r.status_code))
    except Exception as e:
        status.append("🔴 Yeumoney API: " + str(e))
...
async def feedback_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not feedbacks:
        await update.message.reply_text("✅ Khong co feedback nao cho xu ly.")
        return
    text = "📣 <b>Feedback cho xu ly:</b>\n\n"
    for f in feedbacks:
        text += "#" + str(f[0]) + " | User <code>" + str(f[1]) + "</code> | " + f[4][:16] + "\n" + html.escape(f[3][:200]) + "\n\n"
    await update.message.reply_text(text, parse_mode=ParseMode.HTML)
...
async def feedback_done_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        return
    if not context.args:
        await update.message.reply_text("⚠️ Vi du: <code>/feedbackdone 1</code>", parse_mode=ParseMode.HTML)
        return
    try:
    ...
    if isinstance(update, Update) and update.effective_message:
        try:
            await update.effective_message.reply_text(
                "❌ Da xay ra loi. Vui long thu lai sau hoac lien he admin.",
                parse_mode=ParseMode.HTML
            )
        except Exception:
            pass
...
async def post_init(app: Application):
    user_commands = [
        BotCommand("start", "Bat dau / Menu chinh"),
        BotCommand("help", "Huong dan su dung"),
        BotCommand("key", "Nhap key nhan req"),
        BotCommand("model", "Chon model AI"),
        BotCommand("prompt", "Chon tinh cach AI"),
        BotCommand("new", "Xoa lich su chat"),
        BotCommand("history", "Xem lich su chat"),
        BotCommand("profile", "Ho so ca nhan"),
        BotCommand("top", "Bang xep hang"),
        BotCommand("checkin", "Diem danh nhan req"),
        BotCommand("ref", "Ma gioi thieu"),
        BotCommand("feedback", "Gop y cho admin"),
    ]
    admin_commands = [
        BotCommand("admin", "Admin Panel"),
        BotCommand("stats", "Thong ke"),
        BotCommand("logs", "Xem logs"),
        BotCommand("health", "Kiem tra he thong"),
        BotCommand("users", "Danh sach users"),
        BotCommand("user", "Chi tiet user"),
        BotCommand("addreq", "Nap req"),
        BotCommand("ban", "Khoa user"),
        BotCommand("unban", "Mo khoa user"),
        BotCommand("config", "Xem cau hinh"),
        BotCommand("setconfig", "Doi cau hinh"),
        BotCommand("setprice", "Doi gia model"),
        BotCommand("models", "Quan ly model"),
        BotCommand("togglemodel", "Bat/tat model"),
        BotCommand("prompts", "Quan ly prompt"),
        BotCommand("addprompt", "Them prompt"),
        BotCommand("broadcast", "Thong bao"),
        BotCommand("maintenance", "Bat/tat bao tri"),
        BotCommand("cleanup", "Don dep du lieu"),
        BotCommand("feedbacklist", "Xem feedback"),
        BotCommand("feedbackdone", "Xu ly feedback"),
        BotCommand("export", "Xuat CSV"),
    ]
    await app.bot.set_my_commands(user_commands, scope=BotCommandScopeDefault())
    if ADMIN_ID != 0:
        try:
            await app.bot.set_my_commands(user_commands + admin_commands, scope=BotCommandScopeChat(chat_id=ADMIN_ID))
        except Exception:
            pass
...
def main():
    if not BOT_TOKEN or not AI_API_KEY:
        logger.error("Thieu BOT_TOKEN hoac AI_API_KEY!")
        sys.exit(1)
    ...
    logger.info("Denia AI Bot PRO dang khoi dong...")
    asyncio.run(init_db())
    ...
    if ADMIN_TELEGRAM_ID == 0:
        logger.info("Admin auto-set: Nguoi dau tien gui /admin se tro thanh admin")
    else:
        logger.info("Admin ID: " + str(ADMIN_TELEGRAM_ID))
