"""
🤖 DENIA AI BOT — ULTIMATE EDITION v2.0
Professional | Secure | Transparent | Feature-Rich
Author: Refactored by AI Assistant
"""
import os, sys, random, string, logging, asyncio, html as html_module, csv, io, json
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not row:
            ...
            await db.execute(
                ...
                (user_id, username, first_name, DEFAULT_MODEL, ref, datetime.now().isoformat())
            )
            await db.commit()
            return {
...
def make_back_button(data: str = "back_menu") -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Quay lại", callback_data=data)]])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"👋 <b>Xin chào {esc(user.first_name or 'bạn')}!</b>\n\n"
...
async def model_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        if row:
            buttons.append(row)
        buttons.append([InlineKeyboardButton("🔙 Quay lại", callback_data="back_menu")])
        await update.message.reply_text(
...
async def balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔙 Menu Chính", callback_data="back_menu")]
    ]
    await update.message.reply_text(
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "getkey":
        ...
        if not ok:
            await query.edit_message_text(f"⚠️ {msg}", parse_mode=ParseMode.HTML, reply_markup=make_back_button())
            return
        ...
        if not await create_key(user_id, key, req_amount=req_per, source="user"):
            await query.edit_message_text("❌ Lỗi hệ thống khi tạo key. Vui lòng thử lại sau.", parse_mode=ParseMode.HTML, reply_markup=make_back_button())
            return
        ...
        await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=make_back_button(), disable_web_page_preview=True)
        ...
        top = await get_top_users()
        if not top:
            await query.edit_message_text("🏆 Chưa có dữ liệu xếp hạng.", reply_markup=make_back_button())
            return
        text = "🏆 <b>Bảng Xếp Hạng — Top 10</b>\n\n"
        medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]
        for i, u in enumerate(top):
            name = esc(u[1] or u[2] or f"User {u[0]}")
            text += f"{medals[i]} <b>{name}</b> — {u[3]} tin nhắn | 💎{u[4]} | 📤{u[5]}\n"
        await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=make_back_button())
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("📊 Thống kê", callback_data="admin_stats"), InlineKeyboardButton("👤 Quản lý Users", callback_data="admin_users")],
        [InlineKeyboardButton("🔑 Quản lý Key", callback_data="admin_keys"), InlineKeyboardButton("⚙️ Cấu hình", callback_data="admin_config")],
        [InlineKeyboardButton("📢 Broadcast", callback_data="admin_broadcast"), InlineKeyboardButton("🏥 Health Check", callback_data="admin_health")],
        [InlineKeyboardButton("🧹 Dọn dẹp", callback_data="admin_cleanup"), InlineKeyboardButton("📥 Xuất CSV", callback_data="admin_export")],
    ]
    await update.message.reply_text(
...
async def genkey_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin tạo key trực tiếp để tặng"""
    if not is_admin(update.effective_user.id):
        return
    req_amount = int(context.args[0]) if context.args else 100
    key = generate_key()
    if await create_key(0, key, req_amount=req_amount, source="admin", note=f"Created by admin {update.effective_user.id}"):
        await update.message.reply_text(
...
async def admin_keys_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Liệt kê key chưa dùng"""
    if not is_admin(update.effective_user.id):
        return
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT key_code, req_amount, source, created_at, note FROM keys WHERE used = 0 ORDER BY created_at DESC LIMIT 20"
        )
        rows = await cur.fetchall()
    if not rows:
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_stats":
        ...
        await query.edit_message_text(
            ...
            "Tin nhắn sẽ được gửi đến tất cả users.",
            parse_mode=ParseMode.HTML,
            reply_markup=make_back_button("admin_menu")
        )
        ...
        await query.edit_message_text(f"✅ Đã nạp {amount} req cho user <code>{tid}</code>", parse_mode=ParseMode.HTML)
    elif data.startswith("adm_ban|"):
        tid = int(data.split("|")[1])
        await admin_ban_user(tid, 1)
        await query.edit_message_text(f"🚫 Đã khóa user <code>{tid}</code>", parse_mode=ParseMode.HTML)
    elif data.startswith("adm_unban|"):
        tid = int(data.split("|")[1])
        await admin_ban_user(tid, 0)
        await query.edit_message_text(f"✅ Đã mở khóa user <code>{tid}</code>", parse_mode=ParseMode.HTML)
...
def main():
    ...
    # Callbacks
    app.add_handler(CallbackQueryHandler(button_callback, pattern="^(?!adm_|admin_).*"))
    app.add_handler(CallbackQueryHandler(admin_callback_handler, pattern="^(adm_|admin_).*"))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, chat_handler))
    app.add_error_handler(error_handler)

    logger.info("Denia Bot Ultimate v2.0 starting...")
    asyncio.run(init_db())
