"""
🤖 DENIA AI BOT — PRO EDITION v4.0
Zero-bug | Professional Architecture | Profit-Optimized
"""
import os, sys, random, string, logging, asyncio, html as html_module, csv, io
...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "denia_pro.db")
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Seed model prices from env
        for k, v in os.environ.items():
            if k.startswith("PRICE_"):
                mn = k[6:].replace("_", "-").replace("--", "/")
                await db.execute(
                    "INSERT OR IGNORE INTO model_prices (model_name, req_price, enabled, description) VALUES (?, ?, ?, ?)",
                    (mn, int(v), 1, None)
                )
...
async def ensure_user(user_id: int, username: str = None, first_name: str = None) -> Dict[str, Any]:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        ref = f"REF{user_id}{random.randint(1000,9999)}"
...
async def add_log(user_id: int, action: str, detail: str):
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("INSERT INTO logs (user_id, action, detail) VALUES (?, ?, ?)", (user_id, action, detail))
            await db.commit()
    except Exception:
        pass
...
async def add_conversation(user_id: int, role: str, content: str):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        mm = int(await get_cfg("MAX_MEMORY_MESSAGES", str(MAX_MEMORY_MESSAGES)))
...
async def get_conversation_history(user_id: int, limit: int = None) -> List[Dict[str, str]]:
    if limit is None:
        limit = int(await get_cfg("MAX_MEMORY_MESSAGES", str(MAX_MEMORY_MESSAGES)))
    async with aiosqlite.connect(DB_PATH) as db:
...
async def get_stats() -> Dict[str, int]:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        return {"total_users": tu, "total_req": tr, "total_spent": ts, "total_keys": tk, "pending_keys": pk,
                "total_messages": tm, "today_logs": tl, "pending_feedback": pf, "banned_users": tb}
...
async def mark_feedback_done(feedback_id: int, reply: str = None):
    async with aiosqlite.connect(DB_PATH) as db:
        if reply:
            await db.execute("UPDATE feedback SET status = 'done', admin_reply = ? WHERE id = ?", (reply, feedback_id))
        else:
...
async def check_referral(referrer_id: int, referred_id: int) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("UPDATE users SET req_balance = req_balance + ?, req_earned = req_earned + ? WHERE user_id = ?", (bonus, bonus, referrer_id))
        await db.execute("UPDATE users SET req_balance = req_balance + ?, req_earned = req_earned + ? WHERE user_id = ?", (bonus, bonus, referred_id))
        await db.commit()
        return True
...
async def daily_checkin(user_id: int) -> Tuple[bool, int]:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("UPDATE users SET daily_checkin_date = ?, req_balance = req_balance + ?, req_earned = req_earned + ? WHERE user_id = ?",
            (today, bonus, bonus, user_id))
        await db.commit()
        await add_log(user_id, "CHECKIN", f"+{bonus}req")
        return True, bonus
...
async def get_user_keys(user_id: int, limit: int = 10) -> List[tuple]:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT key_code, req_amount, used, created_at, expires_at, source FROM keys WHERE user_id = ? ORDER BY created_at DESC LIMIT ?", (user_id, limit))
        return await cur.fetchall()
...
async def get_pending_keys(limit: int = 20) -> List[tuple]:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT key_code, req_amount, source, created_at, note FROM keys WHERE used = 0 ORDER BY created_at DESC LIMIT ?", (limit,))
        return await cur.fetchall()
        ...
        if last_key_at:
            try:
                ...
                if diff_sec < cooldown * 60:
                    remain = max(1, int((cooldown * 60 - diff_sec) // 60))
...
async def use_key(key_code: str, user_id: int) -> Dict[str, Any]:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT user_id, used, expires_at, req_amount, source, used_by FROM keys WHERE key_code = ?", (key_code,))
        row = await cur.fetchone()
        if not row:
        ...
        await db.commit()
        await add_log(user_id, "USE_KEY", f"{key_code} +{req_amount}req")
...
async def deduct_req(user_id: int, amount: int) -> bool:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
        await add_log(user_id, "CHAT", f"-{amount}req")
        return True
...
async def admin_add_req(user_id: int, amount: int, admin_id: int = 0):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE users SET req_balance = req_balance + ?, req_earned = req_earned + ? WHERE user_id = ?", (amount, amount, user_id))
        await db.commit()
    await add_log(user_id, "ADMIN_ADD", f"Admin {admin_id} +{amount}req")
...
async def create_paste_dpaste(key: str) -> Optional[str]:
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            r = await client.post("https://dpaste.com/api/",
...
async def chat_ai(user_id: int, user_message: str) -> Tuple[str, int]:
    model = (await get_user(user_id) or {}).get("selected_model", DEFAULT_MODEL)
    ...
    for k, v in prices.items():
        if k.lower() in model.lower() or model.lower() in k.lower():
            if v["enabled"]:
                req_cost = v["price"]
            break
    prompt_name = (await get_user(user_id) or {}).get("selected_prompt", "default")
...
async def build_main_menu_text(user_id: int) -> Tuple[str, InlineKeyboardMarkup]:
    u = await get_user(user_id) or await ensure_user(user_id)
    prices = await get_model_prices()
    price_list = "\n".join([f"• <code>{esc(k)}</code>: {v['price']} req" for k, v in prices.items() if v["enabled"]]) or "• Chưa có model."
    text = (
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and len(args) > 0:
        ...
        if arg.startswith("DENIA-"):
            u = await ensure_user(user.id, user.username, user.first_name)
            if u["banned"]:
            ...
            if result["ok"]:
                await update.message.reply_text(
                    ...
                    f"💡 Gõ /help để xem hướng dẫn chi tiết.", parse_mode=ParseMode.HTML)
            else:
    ...
    if u["banned"]:
        await update.message.reply_text("🚫 Tài khoản của bạn đã bị khóa. Vui lòng liên hệ admin.")
        return
    text, markup = await build_main_menu_text(user.id)
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        f"📞 <b>Hỗ trợ:</b> <code>{esc(ADMIN_PHONE)}</code>", parse_mode=ParseMode.HTML)
...
async def cmd_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "Ví dụ: <code>/key DENIA-A1B2C3-D4-E5F6G7H8I9J10-K11</code>", parse_mode=ParseMode.HTML)
        return
...
async def cmd_model(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prices = await get_model_prices()
    if not context.args:
        current = (await get_user(user_id) or {}).get("selected_model", DEFAULT_MODEL)
        buttons = []
        row = []
        for k, v in prices.items():
            if v["enabled"]:
    ...
    await update.message.reply_text(
        ...
        f"💰 Giá: {prices[model_name]['price']} req/tin nhắn", parse_mode=ParseMode.HTML)
...
async def cmd_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        current = (await get_user(user_id) or {}).get("selected_prompt", "default")
        buttons = []
    ...
    await update.message.reply_text(
        ...
        f"AI sẽ trả lời theo phong cách mới từ tin nhắn tiếp theo.", parse_mode=ParseMode.HTML)
...
async def cmd_checkin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        await update.message.reply_text(
            ...
            f"🌟 Hẹn gặp lại bạn vào ngày mai!", parse_mode=ParseMode.HTML)
    else:
        await update.message.reply_text(
    ...
    try:
        if ADMIN_ID and ADMIN_ID != 0:
            await context.bot.send_message(ADMIN_ID,
                f"📣 <b>Feedback mới từ</b> <code>{user_id}</code>:\n{esc(content[:500])}", parse_mode=ParseMode.HTML)
    except Exception:
        pass
...
async def cb_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "menu":
            text, markup = await build_main_menu_text(user_id)
        ...
        if data == "getkey":
            ...
            await query.edit_message_text(
                ...
                parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(back_btn("menu")))
        ...
        logger.error(f"cb_user error: {e}")
        try:
...
async def chat_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ok:
        if remain == -1:
            await update.message.reply_text(
                "🛑 <b>Phát hiện spam!</b>\nBạn gửi tin nhắn quá nhanh. Vui lòng chậm lại.", parse_mode=ParseMode.HTML)
        else:
    ...
    remaining = u_after.get('req_balance', 0) if u_after else 0
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📞 Admin: {esc(ADMIN_PHONE)}", parse_mode=ParseMode.HTML)
...
async def cmd_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    u = await get_user(tid) or await ensure_user(tid)
...
async def cmd_addreq(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(tid,
            ...
            f"Số dư hiện tại đã được cập nhật.", parse_mode=ParseMode.HTML)
    except Exception:
        pass
...
async def cmd_gift(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    await admin_add_req(tid, amount, update.effective_user.id)
    await update.message.reply_text(f"🎁 <b>Đã tặng {amount} req</b> cho user <code>{tid}</code>\nLý do: Admin tặng", parse_mode=ParseMode.HTML)
    try:
        await context.bot.send_message(tid,
            f"🎁 <b>Quà tặng từ Admin!</b>\n\n"
            f"Bạn vừa nhận được <b>{amount} req</b> từ admin.\n"
            f"Hãy tiếp tục trải nghiệm Denia AI nhé!", parse_mode=ParseMode.HTML)
    except Exception:
        pass
...
async def cmd_genkey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await create_key(admin_id, key, req_amount=req_amount, source="admin", note=f"Created by admin {admin_id}"):
        await update.message.reply_text(
            ...
            f"⚠️ Key này dùng được cho <b>bất kỳ ai</b> (1 lần).", parse_mode=ParseMode.HTML)
    else:
...
async def cmd_setconfig(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text(
            ...
            "DAILY_CHECKIN_REQ, REFERRAL_BONUS_REQ, MAINTENANCE_MODE, AUTO_CLEANUP_DAYS", parse_mode=ParseMode.HTML)
        return
...
@safe_handler
async def cmd_togglemodel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    if not context.args:
...
@safe_handler
async def cmd_feedbacklist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
...
@safe_handler
async def cmd_feedbackdone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    if not context.args:
            ...
            if row:
                try:
                    await context.bot.send_message(row[0],
                        ...
                        f"Cảm ơn bạn đã góp ý!", parse_mode=ParseMode.HTML)
                except Exception:
                    pass
    ...
    try:
        if data == "adm_stats":
            ...
            await query.edit_message_text(
                ...
                parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(back_btn("adm_menu")))
...
def main():
    ...
    app.add_handler(CommandHandler("togglemodel", cmd_togglemodel))
    ...
    app.add_handler(CommandHandler("feedbackdone", cmd_feedbackdone))
    app.add_handler(CommandHandler("export", cmd_export))

    # Callbacks — admin first (specific pattern)
    app.add_handler(CallbackQueryHandler(cb_admin, pattern=r"^adm_"))
    app.add_handler(CallbackQueryHandler(cb_user))

    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, chat_handler))
    app.add_error_handler(lambda update, context: logger.error(f"Global error: {context.error}"))

    logger.info("Denia Bot Pro v4.0 starting...")
    asyncio.run(init_db())
